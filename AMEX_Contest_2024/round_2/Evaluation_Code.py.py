...
### Instructions for unstop team : 
'''
You can use this code to measure Round2 performance of submissions by participants.
### Datasets required:
...
dep_var.csv    -> This contains match_id, dataset_type, win_team_id. This is private file available with Unstop team.
...
python Evaluation_Code.py path_to_primary_submission_file path_to_DepVar_file
'''
...
def checkDataType1(df):
    ...
    assert df['win_pred_team_id'].isna().sum() == 0, 'win_pred_team_id should not have NaNs'
    assert df['win_pred_team_id'].dtype == 'int64', ('win_pred_team_id is not int64 type')
    assert df['win_pred_score'].isna().sum() == 0, 'win_pred_score should not have NaNs'
    assert df['win_pred_score'].dtype == 'float64', ('win_pred_score is not float64 type')
    assert df['train_algorithm'].isna().sum() == 0, 'train_algorithm should not have NaNs'
    assert df['train_algorithm'].dtype == 'object', ('train_algorithm is not object type')
    assert df['is_ensemble'].isna().sum() == 0, 'is_ensemble should not have NaNs'
    assert df['is_ensemble'].dtype == 'object', ('is_ensemble is not object type')
    assert df['train_hps_trees'].isna().sum() == 0, 'train_hps_trees should not have NaNs'
    assert df['train_hps_depth'].isna().sum() == 0, 'train_hps_depth should not have NaNs'
    assert df['train_hps_lr'].isna().sum() == 0, 'train_hps_lr should not have NaNs'
    return None
...
# sys.argv takes in the arguments that you pass from terminal. First argument is the location of Evaluation code file, Second argument is the location of primary submission file, Third argument is the location of secondary submission file and Fourth argument is the location of Dependent Variable file
if len(sys.argv) != 3:
  sys.exit("Please pass three files only as mentioned in the Instructions.")

# Location of submission file.
input1_address = sys.argv[1] 
df_input1 = pd.read_csv(input1_address,sep=",",header=0)

### For Unstop Team : Location of Dependent Variable files for Round1 or Round2. This is the private file available with unstop team. These files are comma separated
...
assert df_input1.shape[0] == 207, f'Input file size number of rows incorrect. Expected rowsize 207 not equal to uploaded data rowsize {df_input1.shape[0]}'
...
r2_size = df_input1[df_input1['dataset_type']=='r2'].shape[0]
assert (r2_size == df_round.shape[0]), f'R2 data size in input file is incorrect. Expected rowsize 207 not equal to r2 dataset_type present {r2_size}'

# merging predicted file and dependent variable file
eval_data = pd.merge(df_round,df_input1[df_input1['dataset_type']=='r2'], on=['match id'], how='inner').drop_duplicates()
